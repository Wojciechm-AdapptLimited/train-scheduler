...
tickets = [{
        ...
        "passengers": [
            {"name": p, "seat": s} for p, s in zip(
                random.choices(passengers, k=5),
                random.choices(seats, k=5)
            )]
    } for i, (s1, s2) in enumerate(zip(stations[:-1], stations[1:]))]
trains_n = 10
trains = [{"x":x, "y":y} for x in np.random.uniform(low=50.4683,high=53.8014,size=(trains_n,)) for y in np.random.uniform(low=14.9841,high=22.637,size=(trains_n,))]
reservations = 0
